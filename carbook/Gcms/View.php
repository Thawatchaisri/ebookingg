...
class View extends \Kotchasan\View
{
    ...
    public static function dateRange($item)
    {
        if (
            ...
            if ($begin[1] == $end[1]) {
                return Date::format($item['begin'], 'd M Y').'<br>{LNG_Time} '.Date::format($item['begin'], 'TIME_FORMAT').' {LNG_to} '.Date::format($item['end'], 'TIME_FORMAT');
            } else {
                return Date::format($item['begin']).'<br>{LNG_to} '.Date::format($item['end']);
            }
        }
    }
}
